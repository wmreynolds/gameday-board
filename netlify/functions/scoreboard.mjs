...
export default async (req) => {
  ...
  results.forEach((r, i) => {
    ...
    if (div === "fbs" || div === "fcs") {
      ...
      for (const item of r.value.games ?? []) {
        const g = mapNcaa(item, label);
        if (!g) continue;
        // NCAA's endpoint returns the whole current week; keep only the requested day
        if (g.start && pacificDate(Date.parse(g.start)) !== date) continue;
        games.push(g);
      }
    }
  });
